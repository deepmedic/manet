...
@click.option('--height', default=16, help='width of the image in inches')
...
def write_image(image, mask, overlay, output, height, dpi, linewidth, bbox, contour):
    ...
    plot_2d(image, height=height, mask=mask_arr, bboxes=bboxes,
            overlay=overlay, overlay_cmap='jet', overlay_alpha=0.15, overlay_threshold=0.2, save_as=output, dpi=dpi, linewidth=linewidth)
